...
waveAmpTime = zeros(simu.maxIt+1,1); % size = 39001 
% waveAmpTime = zeros(maxRampIT,1); what I wrote
sizeWaveAmpTime = size(waveAmpTime) % 39001
MAXIT = simu.maxIt
...
% If wave ramping defined
   ...
   for i=1:maxRampIT %size = 100,000
       t = (i-1)*simu.dt;
       

       for j=1:numFqs  % size = 201
           tmp=sqrt(2*Sf(j)*df); % size = 201
           waveAmpTime(i) = waveAmpTime(i) + tmp*real(exp(sqrt(-1)*(frequency(j)*t + waves.phaseRand(j)))); 
       end
