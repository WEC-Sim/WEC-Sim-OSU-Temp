...
Feddy = logsout.getElement('Feddy').Values.Data;
Pfric = logsout.getElement('Pfric').Values.Data;
Peddy = logsout.getElement('Feddy').Values.Data;
...
f = figure;
plot(time,Pabs/1e3,'b',time,Pelec/1e3,'r')
set(findall(gcf,'type','axes'),'fontsize',16)
xlabel('Time (s)')
ylabel('Power (kW)')
...
f = figure;
plot(time,Fpto/1e3,time,Ffric/1e3,time,Feddy/1e3)
set(findall(gcf,'type','axes'),'fontsize',16)
xlabel('Time (s)')
ylabel('Force (kN)')
title('PTO Force, Friction Force, and Eddy Current Force')
legend('Fpto','Ffric','Feddy')

% f = figure;
% plot(time,Pabs/1e3,time,Pelec/1e3,time,Pfric/1e3)
% set(findall(gcf,'type','axes'),'fontsize',16)
% xlabel('Time (s)')
% ylabel('Power (kW)')
% title('Absorbed Power, Electrical Power wo/losses, and Electrical Power w/losses')
% legend('Pabs','PelecWOlosses','Pfric')

f = figure;
plot(time,Pabs/1e3,time,Pelec/1e3,time,(Pelec+(Pfric+Peddy))/1e3)
set(findall(gcf,'type','axes'),'fontsize',16)
xlabel('Time (s)')
ylabel('Power (kW)')
title('Absorbed Power, Electrical Power wo/losses, and Electrical Power w/losses')
legend('Pabs','PelecWOlosses','PelecWlosses')
