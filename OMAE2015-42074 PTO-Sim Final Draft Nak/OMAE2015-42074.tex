...
% Kelley - removed PTO-Sim library figure
...
Electrical power is smaller than mechanical power because the generator block is modeled such that the efficiency of output electrical power is determined by the rotational speed of the generator. In other words, there are some losses in the generator.
...
Current WEC-Sim allows the user to get an idea of what the power production will be; with little knowledge of drivetrains/power electronics, etc. However, if the user wants to design a PCC and compare the benefits of different PCC options, that is where PTO-Sim will come to play. It should be noted that, this obviously requires the user to be more knowledgeable and careful when designing the PTO model. Therefore, the goal of designing PTO-Sim is to abstract the end-user from the details but to make the the wave energy converter more accessible.
