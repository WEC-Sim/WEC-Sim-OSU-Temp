...
\begin{figure}[t]
    \centering
    \includegraphics[width=1\columnwidth]{Images/SimulinkLibraryBrowser}
    \caption{PTO-Sim Simulink Library}
    \label{Lib}
    \end{figure}
...
The PTO was developed and verified with the use of regular waves and the analysis can be focused on understanding the electrical power generated by the system. WEC-Sim is coupled with a direct drive PTO and the relative heave velocity ``zDotRel" is used as an input for the direct drive PTO model. 
...
The PTO force, shown in Fig.~\ref{DD_Fpto}, produced by the ``Electric Machine" model directly follows ``zDotRel" because of the nature of the direct drive design. Figures~\ref{voltage} and~\ref{current} show voltage and current in the external load. It should be noted that the magnitude of the voltage and current depends on the wave climate, and the WEC design parameters \cite{prudell2009novel}.  The absorbed and electric power are shown in Fig.~\ref{DDP}. 
